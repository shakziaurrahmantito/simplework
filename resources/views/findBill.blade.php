...
<div class="col-md-10">
  ...
  <ul class="list-inline">
    <li class="list-inline-item">For test bill no: </li>
    @foreach($getinventorie as $data)
      <li class="list-inline-item">{{$data['billNo']}}</li>
    @endforeach
  </ul>

<form id="findBill" method="post">
  @csrf      
  <div class="form-row">
...
<form method="post" id="myForm">
  ...
  <div class="form-row">
    ...
    <div class="form-group col-md-4">
      ...
      <select class="form-control" id="addProduct" disabled required name="addProduct">
    ...
    <div class="form-group col-md-4">
      <input type="hidden" class="form-control" id="invenId" name="invenId">
    ...
    <hr>
    <button type="submit" class="btn btn-primary float-right">Update change</button>
  </form>
  ...
  <script type="text/javascript">
    ...
    $("#findBill").submit(function(){

        var findBill = $("#findBill").get(0);

        $.ajax({
          url : "{{Route('billNoSearch')}}",
          method: "post",
          data : new FormData(findBill),
          contentType : false,
          processData : false,
          beforeSend : function(){
          ...
          success: function(data){
            ...
            if (data.status == 1) {
                ...
                $("#customerId").html('<option value="'+data.message.customer.id+'" id="customerId">'+data.message.customer.name+'</option>');
                $("#date").val(data.message.date);
                $("#netTotal").val(data.message.totalBillAmount);
                $("#paidAmount").val(data.message.paidAmount);
                $("#dueAmount").val(data.message.dueAmount);
                $("#discountTotal").val(data.message.totalDiscount);
                $("#invenId").val(data.message.id);
